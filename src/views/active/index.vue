    ...
    <c-table ref="table" :handler="getData" @handlerlist="handlerList">
      ...
      >
        ...
        <el-table-column label="操作" width="380">
          <template slot-scope="{row}">
            <el-button type="success" size="mini" icon="el-icon-view" @click.stop="handleClickTo(row.id)">查看</el-button>
            <el-button type="success" size="mini" icon="el-icon-shop" @click.stop="handleClickStatus(row.id)">上/下架</el-button>
            <el-button type="primary" size="mini" icon="el-icon-edit" @click.stop="handleClickEdit(row)">修改</el-button>
            <el-button type="danger" size="mini" icon="el-icon-delete" @click.stop="handleClickDelete(row.id)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </c-table>
...
export default {
  ...
  methods: {
    ...
    handleClickEdit(data) {
      const { id, title, cover, auther, organizer, address, activitytime, detail } = data
      if (cover) {
        this.fileList = [{ name: '封面', url: process.env.VUE_APP_BASE_API + cover }]
      }
      this.form = {
        id, title, cover, auther, organizer, address, detail, activitytime: datetimeFormat2(activitytime)
      }
